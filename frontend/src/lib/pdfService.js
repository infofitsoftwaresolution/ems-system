...
export class PayslipPDFService {
  static generatePayslipPDF(payslip) {
    try {
      ...
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      const periodText = `Salary for the month of ${this.getMonthName(Number(payslip.month))} ${String(payslip.year || '')}`;
      ...
      const empInfo = [
        { label: 'Employee Name', value: String(payslip.employeeName || '') },
        { label: 'Employee ID', value: payslip.employeeId != null ? String(payslip.employeeId) : 'N/A' },
        { label: 'Email', value: String(payslip.employeeEmail || '') },
      ];
      ...
      empInfo.forEach((info, index) => {
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(String(info.value || ''), xPos + 35, yPos);
      });
      ...
      const attendanceInfo = [
        { label: 'Total Days in Month', value: String(payslip.totalDays || 0) },
        { label: 'Working Days', value: String(payslip.workingDays || 0) },
        { label: 'Leave Days', value: String(payslip.leaveDays || 0) },
      ];
      ...
      attendanceInfo.forEach((info, index) => {
        ...
        doc.text(`${info.label}:`, xPos, yPos);
        doc.setFont('helvetica', 'normal');
        doc.text(String(info.value), xPos + 50, yPos);
      });
  ...
  static downloadPayslipPDF(payslip) {
    try {
      ...
      if (!payslip.employeeName || !payslip.month || !payslip.year) {
        throw new Error("Payslip is missing required fields: employeeName, month, or year");
      }

      const doc = this.generatePayslipPDF(payslip);
      const monthName = this.getMonthName(payslip.month);
      const sanitizedName = payslip.employeeName.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_');
      const fileName = `Payslip_${sanitizedName}_${monthName}_${payslip.year}.pdf`;
      doc.save(fileName);
    } catch (error) {
      console.error("Error in downloadPayslipPDF:", error);
      throw error;
    }
  }
  ...
  static getMonthName(month) {
    ...
    return months[monthNum - 1] || String(monthNum || month);
  }
