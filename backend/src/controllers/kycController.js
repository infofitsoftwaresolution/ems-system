...
export class KycController {
  ...
  static generatePermanentEmployeeId() {
    ...
    try {
      ...
      if (
        ...
        if (employee) {
          ...
          return res.status(400).json({
            message:
              "Invalid employee ID provided and could not find employee by name",
  ...
  static async getAllKycRequests(req, res) {
    try {
      ...
      if (status === "approved") {
        try {
          ...
          if (employee) {
            ...
            // Generate permanent employee ID
            const permanentEmployeeId =
              KycController.generatePermanentEmployeeId();
            ...
            console.log(
              `KYC approved for employee ${employee.name}. Permanent ID: ${permanentEmployeeId}`
            );
          } else {
  ...
  static async getKycFile(req, res) {
    try {
